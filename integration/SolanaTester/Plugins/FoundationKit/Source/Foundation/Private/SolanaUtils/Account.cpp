...
Contributors: Jon Sawler, Daniele Calanna, Riccardo Torrisi
*/
...
bool FAccount::IsBase58PrivateKey(const FString& Key)
{
	...
	{
		const TArray<uint8> privateKey = FBase58::DecodeBase58(Key);
...
{
	const TArray<uint8> key = FStringToByteKey(Key);
