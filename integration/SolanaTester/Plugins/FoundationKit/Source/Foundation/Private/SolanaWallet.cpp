...
bool USolanaWallet::SetSaveSlotName(FString NewSaveSlotName)
{
	...
	{
		const UWalletSaveData* SaveData = Cast<UWalletSaveData>(
			UGameplayStatics::LoadGameFromSlot(USolanaWalletManager::GetSlotNamePath(SaveSlotName), 0));
...
bool USolanaWallet::UnlockWallet(FString Password, FText& FailReason)
{
	...
	UWalletSaveData* SaveData = Cast<UWalletSaveData>(
		UGameplayStatics::LoadGameFromSlot(USolanaWalletManager::GetSlotNamePath(SaveSlotName), 0));
...
UWalletAccount* USolanaWallet::GenerateAccountFromGenIndex(int32 GenIndex)
{
	...
	Account = NewObject<UWalletAccount>(this);
	FEd25519Bip39 keypair(Mnemonic.DeriveSeed());
	FAccount AccountData = FAccount::FromSeed(
		keypair.DeriveAccountPath(CurrentSaveData->SelectedDerivationPath.GetDerivationPathSegments(GenIndex)));
...
UWalletAccount* USolanaWallet::ImportAccountFromPrivateKey(FString PrivateKey)
{
	...
	const TArray<uint8> byteKey = FAccount::FStringToByteKey(PrivateKey);
...
void USolanaWallet::UpdateAccounts()
{
	...
	const auto Request = FRequestUtils::RequestMultipleAccounts(GetPublicKeys());
